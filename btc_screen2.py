#!/usr/bin/env python3
"""Segunda pasada BTC 1h: familias mas FRECUENTES (RSI menos extremo, BB angosta, Donchian corto),
sobre BTCUSD HOUR 600d real congelado. Mismos motores reales (bot_gold.signal_last / bot_gold_trend.signal_at)."""
import sys, time
...
print(f"BTCUSD HOUR REAL (pasada 2) | {n} velas | {weeks:.0f} sem | spread {SPREAD}x2")
...
EXITS = [("trail", m, None, False) for m in (3.0, 4.0, 6.0, 8.0)] + \
        [("fixed", a, b, False) for a, b in ((1.0, 2.0), (1.5, 3.0), (2.0, 3.0), (2.0, 4.0))]
rows = []; t0 = time.time()
for bl, bm in ((26, 1.75), (20, 1.5), (14, 2.0)):
    for rlo, rhi in ((40, 60), (45, 55), (35, 65)):
        name = f"REV BB{bl}/{bm} RSI{rlo}/{rhi} foff"
        sg = cache_rev(bl, bm, rlo, rhi, 1000)
        for sname, sides in SIDES.items():
            for kind, p1, p2, ch in EXITS:
...
for ent in (5, 8, 15):
    ...
    for sname, sides in SIDES.items():
        for kind, p1, p2, ch in EXITS + [("trail", m, None, True) for m in (4.0, 8.0)]:
            tr = sim(sg, sides, kind, p1, p2, ch)
            if len(tr) < 60: continue
            tot, wr, pf, mdd, terc, rob = br.stats(tr, "net")
            ex = (f"trail{p1}" if kind == "trail" else f"SL{p1}/TP{p2}") + ("+canal" if ch else "")
            rows.append((rob, tot, pf, wr, mdd, terc, len(tr), f"{name} {sname:<6} {ex}"))
print(f"{len(rows)} configs ({time.time()-t0:.0f}s). == ROB3 (por neto; * = >=3 tr/sem) ==")
...
for rob, tot, pf, wr, mdd, terc, ntr, name in r3[:20]:
...
print("== mejores ROB2 con >=3 tr/sem ==")
for rob, tot, pf, wr, mdd, terc, ntr, name in sorted([r for r in rows if r[0] == 2 and r[6]/weeks >= 3], key=lambda r: -r[1])[:6]:
